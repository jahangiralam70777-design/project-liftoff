  ...
  .handler(async ({ data, context }) => {
    ...
    const [quizRes, qqRes] = await Promise.all([
      ...
      supabase
        ...
        .select(
          "position,mcq:mcqs(id,question,option_a,option_b,option_c,option_d,difficulty)",
        )
...
export const generateCustomExam = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase
      ...
      .select("id,chapter_id,question,option_a,option_b,option_c,option_d,difficulty")
      .eq("status", "published")

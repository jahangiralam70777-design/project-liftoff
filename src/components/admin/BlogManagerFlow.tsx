...
function PostEditorModal({
  ...
  // autosave drafts every 25s when dirty
  useEffect(() => {
    const t = setInterval(() => {
  ...
  // Local revisions: snapshot every 90s when dirty
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (!dirtyRef.current || !form.title.trim()) return;
      setRevisions((prev) => {
        const next = [
          { at: new Date().toISOString(), title: form.title, content: form.content },
          ...prev,
        ].slice(0, 20);
        try { localStorage.setItem(revisionKey, JSON.stringify(next)); } catch {}
        return next;
      });
    }, 90_000);
    return () => clearInterval(t);
          ...
          <ul className="max-h-[60vh] space-y-1.5 overflow-y-auto">
            ...
            {revisions.map((r, i) => (
              <li key={i} className="flex items-center justify-between rounded-lg border border-border/60 bg-background/40 px-3 py-2 text-sm">
                ...
                <button type="button" onClick={() => { if (confirm("Restore this revision? Current draft will be replaced.")) { setF("content", r.content); setF("title", r.title); setRevisionsOpen(false); toast.success("Revision restored"); } }} className="rounded-lg border border-border/60 px-2 py-1 text-xs hover:bg-muted">Restore</button>
              </li>
            ))}
          </ul>
        </DialogContent>
            ...
            >
              ...
              <div className="flex gap-1.5">
                ...
                <IconBtn
                  label="Delete"
                  tone="danger"
                  onClick={() => confirm(`Delete "${c.name}"?`) && delM.mutate(c.id)}
                >
                  <Trash2 className="h-3.5 w-3.5" />
                </IconBtn>
              </div>
